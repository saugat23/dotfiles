...
awful.spawn.with_shell([[
  if pactl list sinks | grep -A10 "$(pactl get-default-sink)" | grep -q "Active Port:.*headphones"; then
    ...
    pactl set-sink-volume @DEFAULT_SINK@ 151%
  fi
]])
...
gears.timer {
  ...
  callback = function()
    awful.spawn.easy_async_with_shell(
      "awk -v FS=' ' '/cpu / {usage=($2+$4)*100/($2+$4+$5)} END {print \" CPU: \" int(usage) \"% \"}' /proc/stat",
      function(out)
        cpu_widget.text = out:gsub("\n", "")
      end
    )
  end
}
...
awful.screen.connect_for_each_screen(function(s)
  ...
  awful.tag({ "home", "term", "browser", "code", "work5", "work6", "work7", "work8", "work9" }, s,
  ...
  local function style_widget(widget, bg_color)
    return wibox.widget {
      {
        widget,
        left   = 8,
...
for i = 1, 9 do
  globalkeys = gears.table.join(globalkeys,
    ...
    -- Bind keypad numbers to tags
    awful.key({ modkey }, "KP_End", function()
        local t = awful.screen.focused().tags[1]; if t then t:view_only() end
      end,
      { description = "view tag 1", group = "tag" }),
    awful.key({ modkey }, "KP_Down",
      function()
        local t = awful.screen.focused().tags[2]; if t then t:view_only() end
      end,
      { description = "view tag 2", group = "tag" }),
    awful.key({ modkey }, "KP_Next",
      function()
        local t = awful.screen.focused().tags[3]; if t then t:view_only() end
      end,
      { description = "view tag 3", group = "tag" }),
    -- ...and so on up to KP_Prior (PageUp) for tag 9
    -- Bind keypad numbers to tags
    awful.key({ modkey }, "KP_Left",
      function()
        local t = awful.screen.focused().tags[4]; if t then t:view_only() end
      end,
      { description = "view tag 4", group = "tag" }),
    awful.key({ modkey }, "KP_Begin",
      function()
        local t = awful.screen.focused().tags[5]; if t then t:view_only() end
      end,
      { description = "view tag 5", group = "tag" }),
    awful.key({ modkey }, "KP_Right",
      function()
        local t = awful.screen.focused().tags[6]; if t then t:view_only() end
      end,
    ...
    awful.key({ modkey }, "KP_Home",
      function()
        local t = awful.screen.focused().tags[7]; if t then t:view_only() end
      end,
      { description = "view tag 7", group = "tag" }),
    awful.key({ modkey }, "KP_Up", function()
        local t = awful.screen.focused().tags[8]; if t then t:view_only() end
      end,
      { description = "view tag 8", group = "tag" }),
    awful.key({ modkey }, "KP_Prior",
      function()
        local t = awful.screen.focused().tags[9]; if t then t:view_only() end
      end,
...
awful.rules.rules = {
  ...
  -- Terminal (Alacritty or Kitty) to workspace 2
  {
    rule = { class = "kitty" },
    properties = { tag = "term" }
  },

  -- IntelliJ IDEA to workspace 4
  {
    rule = { class = "idea" },
    properties = { tag = "code" }
  },
